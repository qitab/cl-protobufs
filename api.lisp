  ...
  (:method ((object structure-object) slot)
    (let* ((class   (type-of object))
      ...
      (assert message ()
              "There is no Protobufs message for the class ~S" class)
      (unless field
        (let* ((lisp-package (proto-real-lisp-package message))
...
(defun condense-protobuf-metadata ()
  ...
  (let (;; fields are hashed by QUAL-NAME, but all slots are compared to the representative
        ;; to determine whether two field metaobjects can be condensed to one.
        (field-hash (make-hash-table :test 'equal)))
    (labels
        ((choose-representative-field (obj)
           (unless (slot-boundp obj 'proto-impl::qual-name)
             (return-from choose-representative-field obj))
           (let* ((name (slot-value obj 'proto-impl::qual-name))
                  (choices (gethash name field-hash))
                  (slots (sb-pcl::std-instance-slots obj)))
             (setf (proto-impl:proto-parent obj) nil)
             (dolist (choice choices (progn (push obj (gethash name field-hash))
                                            obj))
               (when (every #'equal (sb-pcl::std-instance-slots choice) slots)
                 (return choice))))))

      (let ((messages
             (append (loop for v being the hash-value in *messages* collect v)
                     (loop for v being the hash-value in *all-schemas* collect v))))
        (dolist (msg messages)
          (when (and (slot-exists-p msg 'proto-impl::extended-fields)
                     (slot-boundp msg 'proto-impl::extended-fields))
            (mapl (lambda (x) (rplaca x (choose-representative-field (car x))))
                  (proto-impl:proto-extended-fields msg)))
          (when (and (slot-exists-p msg 'proto-impl::fields)
                     (slot-boundp msg 'proto-impl::fields))
            (mapl (lambda (x) (rplaca x (choose-representative-field (car x))))
