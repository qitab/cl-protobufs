...
(deftest extension-serialization-optimized (serialization-suite)
  ...
  (unwind-protect
       (let* ((color1 (make-auto-color :r-value 100 :g-value 0 :b-value 100))
              ...
              (car2   (make-automobile :model "Audi" :color color2))
              (request-2  (make-buy-car-request :auto car2))
              (color3 (make-auto-color :r-value 50 :g-value 60 :b-value 70))
              (car3   (make-automobile :model "BMW" :color color3))
              (request-3  (make-buy-car-request :auto car3)))
         (setf (paint-type color2) :metallic)
         ;; Set boolean extension to NIL (overriding default T) and repeated extensions on color3,
         ;; leaving paint-type unset to test independent multiple extensions.
         (setf (is-matte color3) nil
               (finish-tags color3) '("gloss" "uv-coat")
               (shade-codes color3) '(10 20 30))
         (let ((ser1 (serialize-to-bytes request-1 'buy-car-request))
               (ser2 (serialize-to-bytes request-2 'buy-car-request))
               (ser3 (serialize-to-bytes request-3 'buy-car-request)))
           (assert-false (equal ser1 ser2))
           (assert-equal
               (with-output-to-string (s)
                 (print-text-format request-1 :stream s))
           ...
           (assert-equal
               ...
               (with-output-to-string (s)
                 (print-text-format
                  (deserialize-from-bytes 'buy-car-request ser2) :stream s)))
           (let* ((deser2 (deserialize-from-bytes 'buy-car-request ser2))
                  (dcolor2 (color (auto deser2))))
             (assert-true (has-extension dcolor2 'paint-type))
             (assert-eql :metallic (get-extension dcolor2 'paint-type))
             (assert-false (has-extension dcolor2 'is-matte))
             (assert-false (has-extension dcolor2 'finish-tags))
             (assert-false (has-extension dcolor2 'shade-codes)))
           (let* ((deser3 (deserialize-from-bytes 'buy-car-request ser3))
                  (dcolor3 (color (auto deser3))))
             (assert-false (has-extension dcolor3 'paint-type))
             (assert-true (has-extension dcolor3 'is-matte))
             (assert-false (get-extension dcolor3 'is-matte))
             (assert-true (has-extension dcolor3 'finish-tags))
             (assert-equal '("gloss" "uv-coat") (get-extension dcolor3 'finish-tags))
             (assert-true (has-extension dcolor3 'shade-codes))
             (assert-equal '(10 20 30) (get-extension dcolor3 'shade-codes))))
         (let ((str1 (with-output-to-string (s)
                       (print-text-format request-1 :stream s)))
               (str2 (with-output-to-string (s)
                       (print-text-format request-2 :stream s))))
