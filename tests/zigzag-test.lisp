...
(defun expect-same (msg)
  (assert-true (msg-equalp msg (proto:deserialize-from-bytes
                                'msg (proto:serialize-to-bytes msg)))))

(deftest unsigned-positive (zigzag-suite)
  ;; Small encoding for positive numbers
  (let ((msg (make-msg :u 10)))
    (expect-bytes (list +TAG-U+ 10) (proto:serialize-to-bytes msg))
    (expect-same msg)))
...
(deftest signed-positive (zigzag-suite)
  ...
  (let ((msg (make-msg :s 10)))
    (expect-bytes (list +TAG-S+ (ash 10 1)) (proto:serialize-to-bytes msg))
    (expect-same msg)))
...
(deftest signed-negative (zigzag-suite)
  (let ((msg (make-msg :s -10)))
    ...
    (expect-bytes (list +TAG-S+ (1- (ash 10 1)))
                  (proto:serialize-to-bytes msg))
    (expect-same msg)))

(deftest unspecified-positive (zigzag-suite)
  ;; Small encoding for positive numbers
  (let ((msg (make-msg :i 10)))
    (expect-bytes (list +TAG-I+ 10) (proto:serialize-to-bytes msg))
    (expect-same msg)))
