...
(deftest serialize-regular (alias-tests)
  (let ((obj (protobufs-test-proto::make-message :i 99)))
    (expect-bytes (list +TAG-I+ 99)
                  (serialize-object-to-bytes obj 'protobufs-test-proto::message))))

;; Serialization of the aliased (explicit) message

(defun (:protobuf :serialize protobufs-test-proto::aliased-message) (val buf &aux (size 0))
...
(deftest serialize-aliased (alias-tests)
  (let ((struct (make-aliased-struct :i 99)))
    (expect-bytes (list +TAG-I+ 99)
                  (serialize-object-to-bytes struct 'protobufs-test-proto::aliased-message))))

;;  Serialization of OuterMessage

(deftest serialize-empty-outer (alias-tests)
  (let ((outer (make-instance 'protobufs-test-proto::outer-message)))
...
(deftest serialize-outer-containing-regular (alias-tests)
  (let ((outer (protobufs-test-proto::make-outer-message
                :message (protobufs-test-proto::make-message :i 99))))
