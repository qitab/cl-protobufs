...
(deftest test-method-options (services-suite)
  (let* ((service (proto:find-service-descriptor
    ...
    (assert-eql (values :udp 'string t) (pi::find-option method "protocol"))
    (assert-eql (values 30.0d0 'string t) (pi::find-option method "deadline"))
    (assert-eql (values t 'string t) (pi::find-option method "duplicate_suppression"))
    (assert-eql (values -123 'string t) (pi::find-option method "client_logging"))
    (assert-eql (values :privacy-and-integrity 'string t)
        (pi::find-option method "security_level"))
    (assert-equal (values "admin" 'string t) (pi::find-option method "security_label"))
    (assert-eql (values 42 'string t)
