...
(defun make-tag (type index)
  ...
  (locally (declare #.$optimize-serialization)
    (let ((tag-bits (ecase type
                      ...
                      ((symbol keyword) $wire-type-string)
...
(define-compiler-macro make-tag (&whole form type index)
  "If we know the type at compile time, elide the run-time ECASE."
  (cond ((typep type 'fixnum)
         `(ilogior ,type (iash ,index 3)))
        ((and (listp type)
              (eq (car type) 'quote))
         (let ((wire-type (ecase (second type)
                            ((int32 uint32) $wire-type-varint)
                            ((int64 uint64) $wire-type-varint)
                            ((sint32 sint64) $wire-type-varint)
                            ((fixed32 sfixed32) $wire-type-32bit)
                            ...
                            ((symbol keyword) $wire-type-string)
                            ((date time datetime timestamp) $wire-type-64bit))))
           `(ilogior ,wire-type (iash ,index 3))))
        (t form)))
  ...
  (locally (declare #.$optimize-serialization)
    (i+
     ...
     (ecase type
       ...
       ;; A few of our homegrown types
       ;; TODO(cgay): remove support for KEYWORD. The support for SYMBOL should handle it.
       ((symbol keyword)
...
(define-compiler-macro serialize-scalar (&whole form val type tag buffer)
  "If we recognize TYPE as one of the known scalar types at compile time
   then elide the run-time ECASE." ;; Is it worth it?
  (let* ((type-name (and (listp type)
                         (eq (car type) 'quote)
                         (second type)))
         (encoder (and type-name
                       (get-scalar-encoder-form type-name val buffer))))
    (if encoder
        `(locally (declare #.$optimize-serialization)
...
(define-compiler-macro serialize-packed (&whole form values type index buffer
  ...
  (if (not vectorp-supplied-p)
      form
      (let* ((type-name (and (listp type)
                             (eq (car type) 'quote)
                             (second type)))
             (encode (or (get-scalar-encoder-form type-name 'val buffer)
...
;;; TODO(cgay): this macro changes the semantics of the program. It should
;;; presumably only be an optimization. (Delete it and run the tests to see.)
#+ignore
...
(define-compiler-macro deserialize-packed (&whole form type buffer index)
  (if (not (member type '(int32 uint32 int64 uint64 sint32 sint64
