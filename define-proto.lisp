               ...
               ((define-group)
                (when extra-slot
                  ...
                  (let* ((sname  (field-data-internal-slot-name extra-slot))
                         ...
                         (has-accessor-name (proto-slot-function-name type fname :has))
                         (public-accessor-name (proto-slot-function-name type fname :get)))
                    (collect-form `(without-redefinition-warnings ()
                                     (let ((,stable (tg:make-weak-hash-table
                                                     :weakness :key :test #'eq)))
                                       ,@(and reader
                                              `((defun ,public-accessor-name (object)
                                                  (declare (type ,type object))
                                                  (gethash object ,stable ,default))
                                                (defmethod ,reader ((object ,type))
                                                  (,public-accessor-name object))))
                                       ,@(and writer
                                              `((defun (setf ,public-accessor-name)
                                                    (new-value object)
                                                  (declare (type ,stype new-value)
                                                          (type ,type object))
                                                  (setf (gethash object ,stable) new-value))
                                                (defmethod ,writer ((object ,type) new-value)
                                                  #-ccl (declare (type ,stype new-value))
                                                  (setf (,public-accessor-name object) new-value))))
                                       ...
                                       (defmethod set-extension ((object ,type)
                                                                 ...
                                                                 value)
                                         (setf (gethash object ,stable) value))
                                       (defun ,has-accessor-name (object)
                                         (declare (type ,type object))
                       ...
                       ,@(and writer
                              `((defun (setf ,public-accessor-name)
                                ...
                                (defmethod ,writer ((object ,type) value)
                                  ...
                                  (setf (,public-accessor-name object) value))))
