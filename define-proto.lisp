...
(defmacro define-map (type (&key name index map-desc))
...
Parameters:
  ...
  MAP-DESC: Message name that describes map entries (as generated by protoc)"
  ...
  (let* ((slot      (or type (and name (proto->slot-name name *package*))))
         ...
         (mfield (make-instance 'field-descriptor
                  ...
                  :reader reader))
         (model (find-message map-desc)))
    (record-protobuf-object type model :map)
    `(progn
       define-map
       ,model
       nil
